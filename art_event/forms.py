...
class DeleteProfileForm(forms.Form):
    confirm = forms.BooleanField(
        ...
        help_text=_('Are you sure you want to delete your profile? This cannot be undone.'),
        required=True
    )
...
class ReviewForm(forms.ModelForm):
    rating = forms.ChoiceField(choices=Re_view.RATING_CHOICES,
                               widget=forms.RadioSelect(attrs={'class': 'form-check-input'}))
    level_of_service = forms.ChoiceField(choices=Re_view.LEVEL_CHOICES,
                                         widget=forms.RadioSelect(attrs={'class': 'form-control'}))
    review_text = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control'}))
    wishes = forms.CharField(required=False,
                             widget=forms.TextInput(attrs={'class': 'form-control'}))
    tag = forms.ChoiceField(choices=Re_view.TAG_CHOICES,
                            widget=forms.Select(attrs={'class': 'form-control'}))

    class Meta:
        model = Re_view
        fields = ['reviewer_name', 'helper_name', 'tag', 'rating', 'level_of_service', 'review_text', 'wishes']
        widgets = {
    ...
    def clean(self):
        ...
        if not reviewer_name or not helper_name or not rating or not tag or not level_of_service or not review_text:
            raise forms.ValidationError('All fields are required', code='required_fields')
...
class EventCreationForm(forms.ModelForm):
    ...
    tags = forms.ModelMultipleChoiceField(
        queryset=Tag_article.objects.all(),
...
class HelpForm(forms.ModelForm):
    class Meta:
        model = HelpRequest
        fields = ['user_nick', 'category', 'problem_description', 'district', 'level', 'language', 'contacts']

        widgets = {
            'language': forms.CheckboxSelectMultiple(attrs={'class': 'inline-checkbox'}),
    ...
    category = forms.ModelChoiceField(queryset=Tag_help.objects.all(), label='Category', widget=forms.Select)
    problem_description = forms.CharField(label='Describe problem')
    district = forms.ModelChoiceField(queryset=Region.objects.all(), label='District', widget=forms.Select)
    level = forms.ModelChoiceField(queryset=Level.objects.all(), label='Level', widget=forms.RadioSelect)
    language = forms.MultipleChoiceField(choices=Language.LANGUAGE_CHOICES, label='Language', widget=forms.CheckboxSelectMultiple)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'language' in self.fields:
            ...
            self.fields['language'].widget = forms.CheckboxSelectMultiple(attrs={'class': 'inline-checkbox'})
            self.fields['language'].choices = [(code, language_choices[code]) for code in language_choices]
...
class HelpRequestForm(forms.ModelForm):
    ...
    class Meta:
        model = HelpRequest
        fields = ['user_nick', 'category', 'problem_description', 'district', 'level', 'contacts', 'languages']
        labels = {
...
class HelperCreateForm(forms.ModelForm):
    ...
    class Meta:
        model = Helper
        fields = ('name', 'user_offer_is_free', 'tags', 'support_levels', 'regions', 'contacts', 'languages', 'email', 'phone')
        labels = {
...
class HelperUpdateForm(forms.ModelForm):
    class Meta:
        model = Helper
        fields = ('name', 'user_offer_is_free', 'tags', 'support_levels', 'regions', 'contacts', 'languages', 'email', 'soft_skills', 'phone')
        labels = {
